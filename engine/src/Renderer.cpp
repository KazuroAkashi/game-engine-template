...
Renderer::~Renderer() {
    Engine::log("Destroying renderer...");
...
bool Renderer::setup() {
    Engine::log("Setting up renderer...");
...
void Renderer::attach_shader(const Shader& shader) const {
    Engine::log("Attaching shader to renderer...");
...
void Renderer::render() {
    ...
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex::pos), (void*)offsetof(Vertex, pos));
    ...
    for (auto [entity, mesh, transform] : view.each()) {
        ...
        for (unsigned int i = 0; i < mesh.index_count; i++) { indices[i] = mesh.indices[i] + vertex_count; }
